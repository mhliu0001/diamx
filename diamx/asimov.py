...
class ExtendedBinnedLogLikelihood(LogLikelihoodBase):
    ...
    def set_data(self, d):
        ...
        for base_source in self.base_model.sources:
            assert isinstance(
                base_source, TemplateSource
            ), "ExtendedBinnedLogLikelihood only supports TemplateSource."
        ...
        if len(self.shape_parameters):
            ...
            for sn, base_source in zip(self.source_name_list, self.base_model.sources):
                ...
                if analysis_space_dims is None:
                    ...
                    assert analysis_space_dims == [
                        len(bin) - 1 for bin in bins
                    ], "All sources must have the same analysis space."
                if sn in self.source_morphers:
                    self.ps_interpolators[sn] = self.source_morphers[
                        sn
                    ].make_interpolator(
            ...
            def ps_interpolator(*args):
                ...
                for i, (sn, ps_interpolator) in enumerate(
                    self.ps_interpolators.items()
                ):
                    if sn in self.source_shape_parameters:
                        these_args = np.asarray(
                            [args[0][j] for j in self._get_shape_indices(sn)]
                        )
...
def get_asimov_sigma(
    ...
    for ll_idx, ll in enumerate(old_lls):
        if isinstance(ll, UnbinnedLogLikelihood):
            ...
            rate_multipliers, shape_parameter_settings = (
                extended_binned_llh._kwargs_to_settings(
                    **{
                        k: v
                        for k, v in best_fit_parameters.items()
            ...
            if len(extended_binned_llh.shape_parameters) > 0:
                ...
                for setting_name, (
                    ...
                    log_prior,
                    _,
                ) in extended_binned_llh.shape_parameters.items():
