...
| `appletree/model/pandax4t_run{0,1}_{er,nr}_model.json`, `appletree/instructs/pandax4t_run{0,1}_{er,nr}.json`, `appletree/parameters/pandax4t_{er,nr}_par.json` and `pandax4t_run{0,1}_{er,nr}_par.json` | Detector parameters, micro-physics parameters, and appletree run instructions / parameter priors for the PandaX-4T model. Structure adapted from appletree [[11]](#appletree); $g_1$, $g_{2b}$, drift velocity, electric field, and the P4-NEST recombination Legendre coefficients come from [[16]](#pandax_prd) (per run), and the background rates from Table I of [[15]](#pandax_prl). |
